...
for i in range(len(Input)):
	...
	Input[i] = [ConvertInputNumberToBinary(1, 28)] + Input[i]
	Input[i].append(ConvertInputNumberToBinary(int(28) ,  int(28) ))
...
for i in range(len(Output)):
	Target.append(Output[i][2:])
	Target[i] = [ConvertInputNumberToBinary(1, 315)] + Target[i]
